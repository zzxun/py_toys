...
WORMS_RUNNING = True
STOP = False
# event, make worms to block(pause)
...
class Worm(threading.Thread):  # "Thread" is a class in the "threading" module.
    def __init__(self, name = 'Worm', maxsize = None, color = None, speed = None):
        ...
        # 如果虫子够多，这里可能会挂起，循环不能停止
        # 1.统计剩余格子数目，2.或者设置一个循环次数
        while True:
    ...
    def run(self):
        ...
        while True:
            ...
            if STOP:
                EVENT.clear()  # block until STOP become False
                EVENT.wait()  # if thread being wake up, it will be resume from here

            if not STOP:
            # if can be stop and continue
    ...
    def getNextPosition(self):
        ...
        if self.direction == UP:
            ...
            nexty = self.getNextPos(self.body[HEAD]['y'] - 1, CELLS_HIGH)
        elif self.direction == DOWN:
            nextx = self.body[HEAD]['x']
            nexty = self.getNextPos(self.body[HEAD]['y'] + 1, CELLS_HIGH)
        elif self.direction == LEFT:
            nextx = self.getNextPos(self.body[HEAD]['x'] - 1, CELLS_WIDE)
    ...
    # allow to accross boundary
    def getNextPos(self, pos, max):
        if pos == -1:
            return max - 1
        if pos == max:
            return 0
    ...
    def getNewDirection(self):
        ...
        next = self.getNextPos(y - 1, CELLS_HIGH)
        if next not in (-1, CELLS_HIGH) and GRID[x][next] is None:
            newDirection.append(UP)
        next = self.getNextPos(y + 1, CELLS_HIGH)
        if next not in (-1, CELLS_HIGH) and GRID[x][next] is None:
            newDirection.append(DOWN)
        next = self.getNextPos(x - 1, CELLS_WIDE)
        if next not in (-1, CELLS_WIDE) and GRID[next][y] is None:
...
def handleEvents():
    ...
    for event in pygame.event.get():  # event handling loop
        if not STOP:
            EVENT.set()  # tell worms to wake up
        if (event.type == QUIT) or (event.type == KEYDOWN and event.key == K_ESCAPE):
            WORMS_RUNNING = False  # Setting this to False tells the Worm threads to exit.
            EVENT.set()  # tell worms to wake up and quit
            pygame.quit()
            sys.exit()
        elif (event.type == KEYDOWN and (event.key == K_KP_ENTER or event.key == K_SPACE or event.key == K_RETURN)):
            STOP = not STOP
